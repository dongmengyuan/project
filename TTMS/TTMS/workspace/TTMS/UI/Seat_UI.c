...
void Seat_UI_MgtEntry(int roomID) {
	...
	do {
		...
		case 'U':
		{
			...
			Seat_UI_Modify(list,roomID, row,column);
			break;
			case '2':
				printf("\t\t\t|\t请输入座位的列号:");
				while(1){
				...
				Seat_UI_Modify(list,roomID, 0,column);
				break;
			case '1':
				printf("\t\t\t|\t请输入座位的行号:");
			while(1){
				...
				Seat_UI_Modify(list,roomID, row,0);
				break;
			}
			break;
			}
...
int Seat_UI_Modify(seat_list_t list, int roomID, int row, int column) {
	...
	seat_t rec;
    int i;
    Studio_Srv_FetchByID(roomID,&studioRec);
	if(row==0)
	{
